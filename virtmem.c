...
#define TLB_SIZE 16
#define PAGES 1024
#define PAGE_MASK 0x3FF // 10 1s

#define PAGE_SIZE 1024
#define OFFSET_BITS 10
#define OFFSET_MASK 0xFFC00 // 10 0s followed by 10 1s
...
while (fgets(buffer, BUFFER_SIZE, input_fp) != NULL) {
  ...
  int logical_page = (logical_address & OFFSET_MASK) >> OFFSET_BITS;
  ...
  // Page fault
    ...
    page_faults++;
    // Find next free page in main memory
    physical_page = free_page++;
    // Add new entry to page table
    pagetable[logical_page] = physical_page;
    // Load page from backing store into main memory
    memcpy(main_memory + physical_page * PAGE_SIZE, backing + logical_page * PAGE_SIZE, PAGE_SIZE);
    // Add new entry to TLB
    add_to_tlb(logical_page, physical_page);
  }
  
  // Calculate physical address and value to be read
